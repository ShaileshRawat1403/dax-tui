...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall dax and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g dax-ai",
      pnpm: "pnpm uninstall -g dax-ai",
      bun: "bun remove -g dax-ai",
      yarn: "yarn global remove dax-ai",
      brew: "brew uninstall dax",
      choco: "choco uninstall dax",
      scoop: "scoop uninstall dax",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall dax -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Dax!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# dax") || content.includes(".dax/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".dax/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".dax/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".dax"))
    ) {
      continue
    }
