...
export namespace LSP {
  ...
  const filterExperimentalServers = (servers: Record<string, LSPServer.Info>) => {
    if (Flag.DAX_EXPERIMENTAL_LSP_TY) {
      // If experimental flag is enabled, disable pyright
      if (servers["pyright"]) {
        log.info("LSP server pyright is disabled because DAX_EXPERIMENTAL_LSP_TY is enabled")
  ...
  export namespace Diagnostic {
    export function pretty(diagnostic: LSPClient.Diagnostic) {
      ...
      const severityKey = (diagnostic.severity ?? 1) as 1 | 2 | 3 | 4
