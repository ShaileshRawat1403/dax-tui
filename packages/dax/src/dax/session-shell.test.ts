...
describe("session shell contract", () => {
  test("uses stable user-intent labels for core review and navigation actions", () => {
    expect(SESSION_COMMAND_LABELS.reviewApprovals).toBe("Review approvals")
    expect(SESSION_COMMAND_LABELS.reviewDiff).toBe("Review diff")
    expect(SESSION_COMMAND_LABELS.inspectMcp).toBe("Inspect MCP")
    expect(SESSION_COMMAND_LABELS.reviewDocs).toBe("Review docs")
  ...
  test("defines one role per visible shell surface", () => {
    expect(SESSION_SHELL_ROLES.actionStrip).toBe("What to do now")
    expect(SESSION_SHELL_ROLES.navigator).toBe("Move through this session")
    expect(SESSION_SHELL_ROLES.reviewBar).toBe("Review and inspect")
    expect(SESSION_SHELL_ROLES.sidebar).toBe("Workspace status")
  })
