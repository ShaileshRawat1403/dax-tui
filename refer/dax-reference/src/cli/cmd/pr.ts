...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run dax",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for dax session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found dax session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`dax import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting dax...")
        UI.println()

        // Launch dax TUI with session ID if available
        const { spawn } = await import("child_process")
        const daxArgs = sessionId ? ["-s", sessionId] : []
        const daxProcess = spawn("dax", daxArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          daxProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`dax exited with code ${code}`))
          })
          daxProcess.on("error", reject)
        })
      },
    })
  },
